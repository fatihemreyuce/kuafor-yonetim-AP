...
import { useRevenueSummaryQuery } from "@/hooks/use-salon-revenue-summary";
import { RevenueSummaryEditForm } from "./revenue-summary-form";
import { parseRevenueSummaryIdParam } from "./revenue-summary-utils";

export default function RevenueSummaryEditPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-10">
      <div className="flex flex-wrap gap-2">
        ...
        {data ? (
          <Button variant="ghost" size="sm" asChild>
            <Link to={`/revenue-summary/${data.id}`}>Detay</Link>
          </Button>
        ) : null}
      </div>
      ...
      {isLoading ? (
        ...
        <RevenueSummaryEditForm
          item={data}
          onSuccessNavigate={() => navigate(`/revenue-summary/${data.id}`, { replace: true })}
          onCancel={() => navigate(`/revenue-summary/${data.id}`)}
        />
      )}
    </div>
  );
